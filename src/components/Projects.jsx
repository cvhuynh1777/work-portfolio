const PROJECTS = [
  {
    ...
    tags: ['Python', 'DuckDB', 'Parquet', 'FastAPI', 'claude-sonnet-4-6', 'NASA', 'Delta Lake'],
    href: 'https://github.com/cvhuynh1777/nasa-data-lakehouse',
    color: 'green',
    icon: '◈',
    desc: 'Medallion pipeline ingesting 9,959 asteroid close approaches from NASA\'s NeoWs API. The gold layer enriches each record with a composite hazard score, size classification, and percentile rankings. Paired with a natural language query interface where claude-sonnet-4-6 generates DuckDB SQL from plain English, querying directly over Parquet.',
  },
  {
    title: 'APOD Data Pipeline + RAG Service',
    type: 'AI/ML Engineering',
    group: 'NASA Data Lakehouse',
    tags: ['Python', 'ChromaDB', 'sentence-transformers', 'claude-sonnet-4-6', 'RAG', 'Delta Lake'],
    href: 'https://github.com/cvhuynh1777/nasa-data-lakehouse',
    color: 'green',
    icon: '◈',
    desc: 'RAG pipeline over 1,289 NASA Astronomy Picture of the Day entries. Each entry is embedded with sentence-transformers and stored in ChromaDB. At query time, semantically similar entries are retrieved and passed to claude-sonnet-4-6, which generates a grounded answer citing specific titles and dates. Delta Lake versioning ensures full lineage and time travel across both pipelines.',
  },
  {
    ...
    color: 'green',
    icon: '◉',
    desc: 'Full stack containerized with Podman. MinIO provides S3-compatible storage synced after each pipeline run. Both pipelines run as Airflow DAGs on a daily schedule. A data catalog service auto-generates schema, lineage, and stats per dataset. Deployed to Red Hat OpenShift with API key secrets management.',
  },
  {
    ...
    color: 'green',
    icon: '◈',
    desc: 'Reinforcement learning agent trained to trade NVDA stock, optimizing for drawdown reduction and downside risk control. Experiments tracked with MLflow and the final model packaged as a reproducible Docker artifact.',
  },
  {
